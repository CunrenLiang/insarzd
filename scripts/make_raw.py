...
# Copyright 2010-2014, by the California Institute of Technology. ALL RIGHTS
...
class make_raw(Component, FrameMixin):
    ...
    def calculateSquint(self):
        ...
        if 'a' in self.doppler.quadratic:
            ...
            fd = self.doppler.quadratic[0]*prf
        else:
            self.logger.error(
    ...
    def make_raw(self):
        ...
        try:
            ...
            self.logger.error(
                "make_raw doesn't handle doppler coefficient object, ",
                self.doppler.quadratic
        )
