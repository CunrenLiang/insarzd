...
c     output of resampling     
      ...
      a_rdtmp = rdfval('Input Image File Name','-')
      read(unit=a_rdtmp,fmt='(a)') infile
      a_rdtmp = rdfval('Output Image File Name','-')
      read(unit=a_rdtmp,fmt='(a)') outfile
      a_rdtmp = rdfval('Input Dimensions','-')
      read(unit=a_rdtmp,fmt=*) ndac, nddn
      a_rdtmp = rdfval('Output Dimensions','-')
      read(unit=a_rdtmp,fmt=*) nrac, nrdn
      a_rdtmp = rdfval('Affine Matrix Row 1','-')
      read(unit=a_rdtmp,fmt=*) a, b
      a_rdtmp = rdfval('Affine Matrix Row 2','-')
      read(unit=a_rdtmp,fmt=*) c, d
      a_rdtmp = rdfval('Affine Offset Vector','-')
      read(unit=a_rdtmp,fmt=*) e, f
      a_rdtmp = rdfval('Looks of the Offsets','-')
      read(unit=a_rdtmp,fmt=*) lac, ldn
      a_rdtmp = rdfval('Looks of the Image File','-')
      read(unit=a_rdtmp,fmt=*) lac0, ldn0
      a_rdtmp = rdfval('File Type','-')
      read(unit=a_rdtmp,fmt='(a)') filetype
      a_rdtmp = rdfval('Interpolation Method','-')
      read(unit=a_rdtmp,fmt='(a)') intstyle
